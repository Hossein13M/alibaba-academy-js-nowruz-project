...
function productStock(productId) {
  if (isProductDataValid({ id: productId }) !== true)
    return isProductDataValid({ id: productId });
  let indexOfProduct = findProduct(productId);
  if (indexOfProduct !== undefined)
    return JSON.stringify(warehouse[indexOfProduct]);
  return `product ${productId} is not available!`;
}

function addNewProduct(product) {
  if (isProductDataValid({ ...product }, "create") !== true)
    return isProductDataValid({ ...product }, "create");
  warehouse.push(product);
  return product.name + " is added!";
}

function sellProduct(productId, count = 1) {
  if (isProductDataValid({ id: productId, count }) !== true)
    return isProductDataValid({ id: productId, count });
  let indexOfProduct = findProduct(productId);
  if (indexOfProduct !== undefined) {
    ...
    if (product.enable && isInRange(product.count, count)) {
      ...
      warehouse[indexOfProduct].enable = isProductStockEmpty(product.count)
        ? false
        : product.enable;
...
function changeProductMode(productId, enable) {
  if (isProductDataValid({ id: productId, enable }) !== true)
    return isProductDataValid({ id: productId, enable });
  let indexOfProduct = findProduct(productId);
  if (indexOfProduct !== undefined) {
    let product = warehouse[indexOfProduct];
    enable !== undefined
    ...
    return `${product.name} ${
      warehouse[indexOfProduct].enable ? "enabled" : "disabled"
    }!`;
  }
  return `product ${productId} is not available!`;
}
...
function editProduct(productId, newParams) {
  if (isProductDataValid({ ...newParams }, "edit") !== true)
    return isProductDataValid({ ...newParams }, "edit");
...
function isProductDataValid({ ...params }, mode) {
  ...
  if (
    mode === "create" &&
    checkRequiredDataExists(id, name, price, count) !== true
  )
    return checkRequiredDataExists(id, name, price, count);
  if (id !== undefined && isIdValid(id, mode) !== true)
    return isIdValid(id, mode);
  if (price !== undefined && isCountOrPriceValid(price) !== true)
    return isCountOrPriceValid(price);
  if (count !== undefined && isCountOrPriceValid(count) !== true)
    return isCountOrPriceValid(count);
  return true;
}
...
console.log(
  `count of all the items in the warehouse combined: ${warehouseStock()}`
);
...
console.log(
  `price of all the items that have been sold: ${calcSoldProductsOfWarehouseValue()}`
);
console.log(
  `price of product1 that have been sold: ${calcSoldProductValue(1)}`
);
console.log(
  `price of product5 that have been sold: ${calcSoldProductValue(5)}`
);
console.log(
  `price of product5 that have been sold: ${calcSoldProductValue(2)}`
);
